# NOT RUN {
# Full replications
library(agricolae)#requiere instalar el package agricolae
library(plantbreeding)#Dificil de encontrar usar archivo del paquete
library(reshape)

# Example 1
data(plrv)
model<- with(plrv,AMMI(Locality, Genotype, Rep, Yield, console=FALSE,PC=TRUE))
model$ANOVA
add.anova <- aov(Yield ~ Locality + Rep %in% Locality + Genotype + Locality:Genotype)
results <- ammi.full(dataframe = plrv , environment = "Locality", genotype = "Genotype", replication = "Rep", yvar = "Yield")
# see help(plot.AMMI)
# biplot
plot(model)
# triplot PC 1,2,3 
